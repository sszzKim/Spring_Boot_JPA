...
public interface BoardRepository extends JpaRepository<BoardEntity, Long>{
	...
	@Transactional
	@Modifying //DML을 발생시켰을 때 테이블에 반영된 내용을 엔티티 클래스에 반영, 이것을 안 쓰면 실제 DB에는 값이 바껴있지만 엔티티에는 적용이 안 되어 있다. DML만 쓸 것!!!
	...
	public void hitNoUpdate(@Param("seqno") Long seqno);
	
	//게시물 시퀀스 번호 가져오기
	@Query(value="select tbl_board_seq.nextval from dual",nativeQuery=true)
	public Long getSeqnoNextval();
